...
begin
  ...
  begin
    ...
    check_flow;
    @ (posedge clk);
    if(out_finish !== 1) fail(3);
    #(1);
    if(out_finish === 1) fail(4);
  end
...
task check_reset; begin
    rst_n = 'b0;
    #(20) if(out_finish !== 0 || out_time !== 0) fail(0);
    rst_n = 'b1;
end endtask

task in_time_create; begin
  in_valid = 'b1;
  in_time = $urandom_range(1, 15);
  repeat(1) @(posedge clk);
  out_time_check = in_time;
  #(5);
  in_time = 'bx;
  in_valid = 'b0;
end endtask
...
task check_flow ;
begin
  while(out_time_check)
  begin
    repeat(1) @(posedge clk);
    if(out_time !== out_time_check) fail(2);
    if(out_finish === 1) fail(1);
    out_time_check = out_time_check-1;
  end
end endtask
